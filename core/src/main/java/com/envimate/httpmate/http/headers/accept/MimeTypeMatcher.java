...
import static com.envimate.httpmate.http.headers.accept.MimeTypeElementMatcher.parse;
...
public final class MimeTypeMatcher {
    ...
    static MimeTypeMatcher parseMimeTypeMatcher(final String string) {
        ...
        final MimeType mimeType = parseMimeType(string);

        final MimeTypeElementMatcher type = parse(mimeType.type());
        final MimeTypeElementMatcher subtype = parse(mimeType.subtype());

        return new MimeTypeMatcher(type, subtype);
    }

    public boolean matches(final MimeType mimeType) {
        validateNotNull(mimeType, "mimeType");
