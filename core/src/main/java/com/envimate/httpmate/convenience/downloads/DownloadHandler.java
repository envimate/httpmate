...
public interface DownloadHandler extends HttpHandler {

    @Override
    default void handle(final HttpRequest request,
                        final HttpResponse httpResponse) {
        final Download download = handle(request);
        download.mapToResponse(httpResponse);
    }

    Download handle(HttpRequest httpRequest);
}
